...
#define NUM_LEDS3       33 + 41 + 33 + 41 //fist to 108*4
...
#define NUM_LEDS3_FIST  108 + 108 + 108 + 108 //COMBAT - 2 paski z przodu po 70 LED
...
void LedGameStart(void) {
  ...
  if(Fram.BoxerMat==1) {
    ...
    if (Fram.BoxerModel==FIST) {
      dot_beat(&leds3[108], 108);
      dot_beat(&leds3[108*2], 108);
      NightRiderRGB(&leds3[0], 108);
      NightRiderRGB(&leds3[108*3], 108);
...
void LedPercent(uint16_t value)
{
  ...
  if ((Fram.BoxerModel == HIT) || (Fram.BoxerModel == GIFT) || (Fram.BoxerModel == GIFT_ESPANIOL)) {
    ...
    fill_solid( &leds3[NUM_LEDS3_MONSTER-peak], peak, CRGB(128,128,128));
  	...
  	MAX_PEAK = 108;
  	peak = (value * MAX_PEAK) / 100;
    //StroboScope(&leds3[0], 108, CRGB::Green);
    fill_rainbow( &leds3[108], peak, starthue, 20);
    fill_rainbow( &leds3[((108*3)-peak)], peak, starthue, 20);
    fadeLightBy(leds3, num_leds3, 50);
    //StroboScope2(&leds3[108*3], 108, CRGB::Green);
...
/*
  if(middle<NUM_LEDS) leds[108-middle] = CRGB::Purple;
  if(inner<NUM_LEDS) leds[108-inner] = CRGB::Blue;
  if(outer<NUM_LEDS) leds[outer] = CRGB::Aqua;
*/
