...
{  
  ...
  if(KEY_START_HAMMER || QuickStartHammer) {
    QuickStartHammer = false;
    printf("\n KEY_START_HAMMER");
    GameMode = HAMMER;
    ret = true;
...
bool Pomiar()
{
  ...
  if (wynik == 0)
  {
    ...
    if(Fram.BoxerMat==1) {
      ...
      fill_rainbow(leds1, num_leds1, starthue, 20);
      if(Fram.BoxerModel==FIST) {
        fill_rainbow(&leds3[0], 108, starthue, 10);
        fill_rainbow(&leds3[108*3], 108, starthue, 10);
        starthue+=2;
        fill_solid(&leds3[108*1], 108, CRGB::Red);
        fill_solid(&leds3[108*2], 108, CRGB::Red);
        
        uint8_t inner = beatsin8(30, 108/4, 108/4*3);    // Move 1/4 to 3/4
        uint8_t outer = beatsin8(30, 0, 108-1);               // Move entire length
        uint8_t middle = beatsin8(30, 108/3, 108/3*2);   // Move 1/3 to 2/3

        if(middle<108) leds3[108+middle] = CRGB::White;
        if(inner<108) leds3[108+inner] = CRGB::Blue;
        if(outer<108) leds3[108+outer] = CRGB::Black;
        if(middle<108) leds3[2*108+middle] = CRGB::White;
        if(inner<108) leds3[2*108+inner] = CRGB::Blue;
        if(outer<108) leds3[2*108+outer] = CRGB::Black;
        fadeLightBy(leds3, num_leds3, 50);
      }
